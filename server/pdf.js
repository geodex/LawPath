...
async function generateContractPdf(options) {
  ...
  return new Promise((resolve, reject) => {
    const chunks = [];
    // bufferPages keeps every page reachable for the footer pass below. Without
    // it PDFKit flushes page 0 the moment page 1 starts, and switchToPage(0)
    // throws — so every document long enough to need a second page failed to
    // download while one-pagers sailed through.
    const doc = new PDFDocument({ size: "A4", margin: 60, bufferPages: true, info: { Title: title, Author: tenantProfile.tradingName || "LawPath SA" } });
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      doc.switchToPage(i);
      // The footer sits inside the bottom margin. PDFKit auto-paginates any
      // text written past (height - margin.bottom), so without zeroing the
      // margin every footer write spawned a phantom trailing page.
      ...
      doc.fontSize(7).fillColor(BRAND_MUTED).font("Helvetica")
        .text(`${firmName}  |  Generated by LawPath SA  |  Page ${i + 1} of ${range.count}`, 60, footerY, { width: pageWidth, align: "center" });
    }
...
async function generateInvoicePdf({ invoice, lineItems = [], payments = [], tenantProfile = {} }) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    // bufferPages: same multi-page footer crash as generateContractPdf — a
    // long WIP invoice spilling to page 2 would fail to download at all.
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      doc.switchToPage(i);
      // Zero the bottom margin: the footer writes inside it, and PDFKit
      // auto-paginates past-margin text onto a phantom trailing page.
      doc.page.margins.bottom = 0;
      ...
      doc.fontSize(7).fillColor(BRAND_MUTED).font("Helvetica")
        .text(`${firmName}  |  ${invoice.invoice_number || invoice.invoiceNumber}  |  Generated by LawPath SA  |  Page ${i + 1} of ${range.count}`, 60, footerY, { width: pageWidth, align: "center" });
    }
